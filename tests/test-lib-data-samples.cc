...
CPPUNIT_ASSERT_THROW(samples.add(getSample(pt::ptime(sampleDate, pt::time_duration(11,30,00)) + gr::days(1))), bo::exception::Base);
  ...
  samples.appendToFile("AndreasW_%Y%m%d.bos");

  samples.readFromFile("test.bos");
...
void SampleTest::testUniq() {
  bo::data::Samples::P samples1 = getSamples1();
  bo::data::Samples::P samples2 = getSamples2();

  //samples1->add(samples2);

  //samples1->uniq();

  CPPUNIT_ASSERT_EQUAL(31, samples1->size());
  CPPUNIT_ASSERT_EQUAL(0, samples2->size());
}

void SampleTest::testMerge() {
  bo::data::Samples::P samples1 = getSamples1();
  bo::data::Samples::P samples2 = getSamples2();

  //samples1->merge(samples2);

//  std::cout << "samples1:\n";
//  samples1.print(bo::data::Samples::VERBOSE_WITH_TIMESTAMP);
//  std::cout << "samples2:\n";
//  samples2.print(bo::data::Samples::VERBOSE_WITH_TIMESTAMP);

  CPPUNIT_ASSERT_EQUAL(31, samples1->size());
  CPPUNIT_ASSERT_EQUAL(0, samples2->size());

  bo::data::Samples::P samples3 = getSamples1();
  bo::data::Samples::P samples4 = getSamples2();
  bo::data::Samples::P samples5 = getSamples2();
  //samples4->add(samples5);

  //samples4->merge(samples3);

  //std::cout << "samples1:\n";
  //samples4.print(bo::data::Samples::VERBOSE_WITH_TIMESTAMP);
  //std::cout << "samples2:\n";
  //samples2.print(bo::data::Samples::VERBOSE_WITH_TIMESTAMP);

  CPPUNIT_ASSERT_EQUAL(0, samples3->size());
  CPPUNIT_ASSERT_EQUAL(31, samples4->size());
}
